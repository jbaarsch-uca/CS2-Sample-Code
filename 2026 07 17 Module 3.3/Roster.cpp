...
void Roster::setStudents(Student* students) {
...
Roster& Roster::operator+(Roster& otherRoster) {
	...
	int classSize = size + otherRoster.size;
	Student* bigClass = new Student[classSize];
	for (int i = 0; i < size; i++)
		bigClass[i] = (*this->getStudent(i));
	for (int i = 0; i < otherRoster.size; i++)
		bigClass[size + i] = *(otherRoster.getStudent(i));
	// Old Version
//	Roster roster = Roster(courseName + otherRoster.courseName, bigClass,
//		classSize);
//	return roster;

	// New Version
	Roster* roster = new Roster(courseName + otherRoster.courseName, bigClass,
		classSize);
	return *roster;
}
Roster Roster::operator-(Roster& otherRoster) {
